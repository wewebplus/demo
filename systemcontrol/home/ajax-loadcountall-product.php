...
$sql .= "(";
	...
	$sql .= " ,SUM(IF(DATE("._TABLE_PRODUCTS_."_CreateDate) BETWEEN '".$DMStart."' AND '".$DMEnd."', 1, 0)) AS CountDataAll";
	$sql .= " ,SUM(IF(DATE("._TABLE_PRODUCTS_."_CreateDate) BETWEEN '".$DRStart."' AND '".$DREnd."', 1, 0)) AS CountDataFix";
	if(count($dateranginweek)>0){
		foreach($dateranginweek as $kldate=>$vldate){
			$sql .= " ,SUM(IF(DATE("._TABLE_PRODUCTS_."_CreateDate) BETWEEN '".$vldate["monday"]."' AND '".$vldate["sunday"]."', 1, 0)) AS DW0".$kldate;
		}
	}
  $sql .= " ,SUM(IF("._TABLE_PRODUCTS_."_TypeID = '1', 1, 0)) AS Sum01";
	$sql .= " ,SUM(IF("._TABLE_PRODUCTS_."_TypeID = '2', 1, 0)) AS Sum02";
	$sql .= " ,SUM(IF("._TABLE_PRODUCTS_."_TypeID = '3', 1, 0)) AS Sum03";
	$sql .= " ,SUM(IF("._TABLE_PRODUCTS_."_TypeID = '4', 1, 0)) AS Sum04";
	$sql .= " ,SUM(IF("._TABLE_PRODUCTS_."_TypeID = '1' AND DATE("._TABLE_PRODUCTS_."_CreateDate) BETWEEN '".$DRStart."' AND '".$DREnd."', 1, 0)) AS Sum01Fix";
  $sql .= " ,SUM(IF("._TABLE_PRODUCTS_."_TypeID = '2' AND DATE("._TABLE_PRODUCTS_."_CreateDate) BETWEEN '".$DRStart."' AND '".$DREnd."', 1, 0)) AS Sum02Fix";
	$sql .= " ,SUM(IF("._TABLE_PRODUCTS_."_TypeID = '3' AND DATE("._TABLE_PRODUCTS_."_CreateDate) BETWEEN '".$DRStart."' AND '".$DREnd."', 1, 0)) AS Sum03Fix";
	$sql .= " ,SUM(IF("._TABLE_PRODUCTS_."_TypeID = '4' AND DATE("._TABLE_PRODUCTS_."_CreateDate) BETWEEN '".$DRStart."' AND '".$DREnd."', 1, 0)) AS Sum04Fix";
	$sql .= " FROM "._TABLE_PRODUCTS_." AS p";
	$sql .= " WHERE DATE(p."._TABLE_PRODUCTS_."_CreateDate) BETWEEN '".$DMStart."' AND '".$DMEnd."'";
$sql .= ") TB";
// echo $sql;
// exit();
$z = new __webctrl;
...
$found["dataproductcountformat"] = number_format($AllCountData);
$found["dataproducttext"] = dateformat($DMStart." 00:00:00",'j F Y','Thai')." ถึงปัจจุบัน";
$found["dataproductcount"] = $AllCountData;
// $found["dataproduct"] = $foundinner;
