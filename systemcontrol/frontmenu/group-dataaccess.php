...
switch($actiontype){
	case 'changestatus':
		$itemid = $itemid;
		$StatusTo = $statusto;
		$sql = "SELECT "._TABLE_FRONTMENU_."_ID AS ID,"._TABLE_FRONTMENU_."_Status AS ListStatus,"._TABLE_FRONTMENU_."_Language AS InLanguage,"._TABLE_FRONTMENU_."_Folder AS InFolder FROM "._TABLE_FRONTMENU_." WHERE "._TABLE_FRONTMENU_."_ID = ".(int)$itemid;
		$z = new __webctrl;
		$z->sql($sql);
		$v = $z->row();
		$Row = $v[0];
		...
		$update[_TABLE_FRONTMENU_."_Status"] = "'".$StatusTo."'";
		$z = new __webctrl;
		$z->update(_TABLE_FRONTMENU_,$update,array(_TABLE_FRONTMENU_."_ID=" => $itemid));
		unset($update);
		if($StatusTo=='On'){
