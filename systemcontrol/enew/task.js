function loadpageajax(page){
	var json = (function () {
		...
		$.ajax({
			...
			'url': 'task-ajax-loadpage-json.php',
			'dataType': "json",
	...
	html +='<tr>';
		...
		html +='<th class="w75">สมาชิก</th>';
		html +='<th class="w75">ส่ง</th>';
		html +='<th class="w150">Date</th>';
		html +='<th class="w150">Send Date</th>';
	...
	if(len>0){
      ...
      //var numberindex = json['result'][i].ListIndex;
			...
			html +='<tr class="listitem" data-z="'+numberindex+'">';
				...
				html += '<td class="tasksubject"><div>'+json['result'][i].Subject+'</div><div class="docname">(Document Name : '+json['result'][i].DocSubject+')</div></td>';
				html += '<td><span>'+json['result'][i].NoOfMember+'</span></td>';
				html += '<td><span>'+json['result'][i].NoOfSend+'</span></td>';
				html += '<td class="">'+json['result'][i].Dateshow+'</td>';
				html += '<td class="">'+json['result'][i].SendDateShow+'</td>';
	            ...
	            action: function () {
								var json = (function () {
									...
									$.ajax({
										...
										'url': FullPath+'enew/task-dataaction.php',
										'dataType': "json",
										'success': function (data) {
											json = data;
										},
...
function changeStatus(t){
	...
	$.ajax({
		...
		'type':'POST',
		'data': {MyData:mydata},
		'url': PathURL+"enew/task-dataaccess.php",
		'success': function (data) {
...
function clicktoreset(t){
	$.confirm({
			title: 'Confirm!',
			content: 'Are you sure to reset this record?',
			buttons: {
				Submit: {
						text: 'Confirm',
						btnClass: 'btn-blue',
