...
foreach($_FILES as $index => $file){
  ...
  if(!empty($fileTempName) && is_uploaded_file($fileTempName))  {
    ...
    $sql = "SELECT MAX("._TABLE_INGREDIENTS_FILE_."_Order) AS MaxO FROM "._TABLE_INGREDIENTS_FILE_." WHERE 1 ";
		$sql .= " AND "._TABLE_INGREDIENTS_FILE_."_ContentID = ".(int)$itemid;
		if($sessionid<>""){
		...
		$Row = $v[0];
		$MaxOrder = $Row["MaxO"]+1;
		$insert[_TABLE_INGREDIENTS_FILE_."_ContentID"] = "'".sql_safe($itemid)."'";
		$insert[_TABLE_INGREDIENTS_FILE_."_Session"] = "'".sql_safe($sessionid)."'";
		$insert[_TABLE_INGREDIENTS_FILE_."_Subject"] = "'".sql_safe($fileName)."'";
		$insert[_TABLE_INGREDIENTS_FILE_."_FileName"] = "'".sql_safe($NewFile)."'";
		$insert[_TABLE_INGREDIENTS_FILE_."_FileType"] = "'".sql_safe($myExtension)."'";
		$insert[_TABLE_INGREDIENTS_FILE_."_FileSize"] = "'".sql_safe($FileSize)."'";
		$insert[_TABLE_INGREDIENTS_FILE_."_Views"] = "'0'";
		$insert[_TABLE_INGREDIENTS_FILE_."_CreateByID"] = "'".(int)$_SESSION['Session_Admin_ID']."'";
		$insert[_TABLE_INGREDIENTS_FILE_."_CreateDate"] = "NOW()";
		$insert[_TABLE_INGREDIENTS_FILE_."_Order"] = "'".sql_safe($MaxOrder)."'";
    $insert[_TABLE_INGREDIENTS_FILE_."_Flag"] = "'".sql_safe($myflag)."'";
		$z = new __webctrl;
		$z->insert(_TABLE_INGREDIENTS_FILE_,$insert);
		unset($insert);
