...
$PathUploadFile = (isset($defaultdata[$Login_MenuID]["path"]["FILERES"])?$defaultdata[$Login_MenuID]["path"]["FILERES"]:_RELATIVE_TEMP_UPLOAD_);
...
$ArrField[] = _TABLE_RESTAURANT_FILE_."_ContentID AS CID";
$ArrField[] = _TABLE_RESTAURANT_FILE_."_Session AS Session";
$ArrField[] = _TABLE_RESTAURANT_FILE_."_Subject AS Detail";
...
$sql = "SELECT ".implode(",",$ArrField)." FROM "._TABLE_RESTAURANT_FILE_." WHERE "._TABLE_RESTAURANT_FILE_."_ID = ".intval($itemid);
unset($arrf);
$z = new __webctrl;
$z->sql($sql);
$v = $z->row();
...
$Flag = $Row["Flag"];
if($Flag=='restaurant_image' || $Flag=='restaurant_exterior_photographs' || $Flag=='restaurant_interior_photographs'){
...
if(intval($ContentID)>0){
	if($AddIndex=="Old"){
		$myfile = $PathUpload.$FileName;
	}else{
		$myfile = $PathUpload.$ContentID."/".$FileName;
	}
}else{
	$myfile = $PathUpload.$SessionID."/".$FileName;
}
