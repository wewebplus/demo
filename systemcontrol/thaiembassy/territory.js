function loadpageajax(page){
	var json = (function () {
		...
		$.ajax({
			...
			'url': 'territory-ajax-loadpageindex-json.php',
			'dataType': "json",
  ...
  html +='<thead>';
    html +='<tr>';
		html +='<th class="w40 text-center">';
			...
			html +='</label>';
		html += '</th>';
		html +='<th class="w200 text-left">Region</th>';
		html +='<th class="">Name</th>';
		html +='<th class="w125 text-left">Country</th>';
		html +='<th class="w300 text-left">ภายในประเทศที่ประจำการ</th>';
		html +='<th class="w300 text-left">ภายนอกประเทศที่ประจำการ</th>';
		...
		for (var i = 0; i< len; i++) {
			...
			html +='<tr>';
				...
				html +='<td class="linetd">';
					html +='<div>'+json['result'][i].RegionName+'</div>';
			  html +='</td>';
			  html +='<td class="linetd">';
					html +='<div>'+json['result'][i].Name+'</div>';
			  html +='</td>';
				html +='<td class="linetd">';
					html +='<div>'+json['result'][i].CountryName+'</div>';
			  html +='</td>';
				html +='<td class="linetd">';
					html +='<div>'+json['result'][i].vInternal+'</div>';
				html +='</td>';
				html +='<td class="linetd">';
					html +='<div>'+json['result'][i].vExternal+'</div>';
			  html +='</td>';
	            ...
	            action: function () {
								var json = (function () {
									...
									$.ajax({
										...
										'url': FullPath+'thaiembassy/territory-dataaction.php',
										'dataType': "json",
										'success': function (data) {
											json = data;
										},
...
function changeStatus(t){
	...
	$.ajax({
		...
		'type':'POST',
		'data': {MyData:mydata},
		'url': PathURL+"thaiembassy/territory-dataaccess.php",
		'dataType': "json",
		'success': function (data) {
			if(data.status=='OK'){
...
function clicktoexportexcel(t){
	var LoginData = $('#ajaxFrm #LoginData').val();
	var dwnfile = "printexcelterritory.php?"+LoginData;
	var triggerDownload = $("<a>").attr("href", dwnfile).attr('target', '_blank').appendTo("body");
	triggerDownload[0].click();
	triggerDownload.remove();
}
function loadajaxselectcountry(t){
	var tb = $(t);
	var thisval = tb.val();
	// $('select[name=selectCountry_I]').append(thisval).trigger('change');
	$('select[name=selectCountry_I]').val(thisval);
	$('select[name=selectCountry_I]').trigger('change');
	// loadajaxstate_I($('select[name=selectCountry_I]'));
}
...
function loadajaxstate_I(data){
	var tb = $(data);
	var selectto = $('#selectProvince_I');
	// console.log(data);
	var FullPath = $('#ajaxFrm input[name=PathURL]').val();
	// console.log(FullPath);
	var LoginData = $('#ajaxFrm #LoginData').val();
...
function loadajaxstate_E(data){
	var tb = $(data);
	var selectto = $('#selectProvince_E');
	// console.log(data);
	var FullPath = $('#ajaxFrm input[name=PathURL]').val();
	// console.log(FullPath);
	var LoginData = $('#ajaxFrm #LoginData').val();
