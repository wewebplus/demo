function loadpagefileajax(saveData,to,type){
	...
	var json = (function () {
		...
		$.ajax({
			...
			'url': './pending-ajax-loadpagefilelist.php',
			'dataType': "json",
...
function deletelistfile(t){
	var tb = $(t);
	var itemid = tb.attr('rel');
	var maindiv = tb.closest('.bs-component');
	var InData = maindiv.find('input[name^=saveData]');
	var loaddataname = InData.attr('name');
	let resultdataname = loaddataname.replace("saveData", "");
	var saveData = InData.val();
	$.confirm({
			title: 'Confirm!',
			...
			buttons: {
				Submit: {
						...
						action: function () {
							...
							function(data){
								if(data == 1){
									loadpagefileajax(saveData,'#outputuploadFile'+resultdataname,'edit');
								}else{
...
function DownloadDocument(t){
	var data = $(t).attr('rev');
  var url = 'pending-downloadsrc.php?'+data;
