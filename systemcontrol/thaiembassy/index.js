function loadpageajax(page){
  ...
  html +='<thead>';
    html +='<tr>';
		...
		html +='<th class="w125 text-left"> </th>';
		...
		for (var i = 0; i< len; i++) {
			...
			html +='<tr>';
			  ...
			  html +='<td class="linetd">';
					...
					html +='<p>Country : '+json['result'][i].CountryName+'</p>';
					html +='<p>สคต. : '+json['result'][i].TerritoryName+'</p>';
			  html +='</td>';
				html +='<td class="linetd">';
					html +='<p>Email : '+json['result'][i].valueEmail+'</p>';
					html +='<p>Tel : '+json['result'][i].valueTel+'</p>';
			  html +='</td>';
	            ...
	            action: function () {
								var json = (function () {
									...
									$.ajax({
										...
										'url': FullPath+'thaiembassy/index-dataaction.php',
										'dataType': "json",
										'success': function (data) {
											json = data;
										},
