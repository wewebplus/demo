function loadpageajax(page){
	...
	var json = (function () {
		...
		var dataEmpType = $('.form-search select[name=selectEmpType]').val();
		$.ajax({
			'async': false,
			'global': false,
			'type':'POST',
			'data':{page:page,LoginData:LoginData,dataKeyword:dataKeyword,dataEmpType:dataEmpType},
			'url': 'ajax-loadpageemployee-json.php',
			'dataType': "json",
  ...
  html +='<thead>';
    html +='<tr>';
			html +='<th class="w40 text-center">';
				...
				html +='</label>';
			html += '</th>';
      html +='<th class="w150">'+Array_Mod_Lang["txtinput:Employee ID"][actionlang]+'</th>';
      html +='<th class="w250">'+Array_Mod_Lang["txtinput:fullname"][actionlang]+'</th>';
      html +='<th class="">'+Array_Mod_Lang["txtinput:useremail"][actionlang]+'</th>';
			html +='<th class="">'+Array_Mod_Lang["txtinput:Emp_Country"][actionlang]+'</th>';
			html +='<th class="w150 text-left">'+Array_Mod_Lang["txtinput:Emp_Type"][actionlang]+'</th>';
  ...
  html +='<tbody>';
	if(len>0){
		for (var i = 0; i< len; i++) {
			...
			var countRef = parseInt(json['result'][i].CountRef);
			html +='<tr>';
				...
				html +='</td>';
			  html +='<td>'+json['result'][i].valueCode+'</td>';
			  html +='<td>';
			  ...
			  html +='</td>';
			  html +='<td class="">'+json['result'][i].valueEmail+'</td>';
				html +='<td class="">'+json['result'][i].CountryName+'</td>';
				html +='<td class="">'+json['result'][i].InType+'</td>';
			  html +='<td class="text-left">';
				html +='<div class="btn-group text-left">';
				  html +='<button type="button" class="btn '+json['result'][i].valueStatusCss+' br2 fs12 dropdown-toggle" data-toggle="dropdown" aria-expanded="false"> ';
					html += json['result'][i].valueStatustxt;
	            ...
	            action: function () {
								var json = (function () {
									...
									$.ajax({
										...
										'url': FullPath+'employee/index-dataaction.php',
										'dataType': "json",
										'success': function (data) {
											json = data;
										},
...
function changeStatus(t){
	...
	$.ajax({
		...
		'type':'POST',
		'data': {MyData:mydata},
		'url': PathURL+"employee/index-dataaccess.php",
		'success': function (data) {
