...
if($itemid>0){
  foreach($systemLang as $lkey=>$lval){
    ...
    savetxt($PathUploadHtml.$html01,$content);
  	...
  	if($detailid>0){
  		$sqlx = "SELECT "._TABLE_FAQ_DETAIL_."_HTMLFileName AS HTMLFileName FROM "._TABLE_FAQ_DETAIL_." WHERE "._TABLE_FAQ_DETAIL_."_ID = ".(int)$detailid;
  		$zx = new __webctrl;
  		$zx->sql($sqlx);
  		$vx = $zx->row();
  		$rowx = $vx[0];
      ...
      if(is_file($oldhtml01)){unlink($oldhtml01);}
  		$update[_TABLE_FAQ_DETAIL_.'_Subject'] = "'".sql_safe($inputSubject)."'";
  		$update[_TABLE_FAQ_DETAIL_.'_Title'] = "'".sql_safe($inputTitle)."'";
  		$update[_TABLE_FAQ_DETAIL_.'_HTMLFileName'] = "'".sql_safe($html01)."'";
  		$update[_TABLE_FAQ_DETAIL_.'_UpdateDate'] = "NOW()";
  		if(!empty($Ignore)){
  			$update[_TABLE_FAQ_DETAIL_.'_Status'] = "'Off'";
  			$Lang .= "|".$lkey.":Off";
  		}else{
  			$update[_TABLE_FAQ_DETAIL_.'_Status'] = "'On'";
  			$Lang .= "|".$lkey.":On";
  		}
  		$z = new __webctrl;
  		$z->update(_TABLE_FAQ_DETAIL_,$update,array(_TABLE_FAQ_DETAIL_."_ID=" => (int)$detailid));
  		unset($update);
  	}else{
  		$insert[_TABLE_FAQ_DETAIL_.'_ContentID'] = "'".sql_safe($itemid)."'";
  ...
  $sql = "SELECT "._TABLE_FAQ_."_Picture AS thumb,"._TABLE_FAQ_."_PictureHome AS thumbhome FROM "._TABLE_FAQ_." WHERE "._TABLE_FAQ_."_ID = ".(int)$itemid;
  $z = new __webctrl;
  $z->sql($sql);
  $v = $z->row();
  $row = $v[0];
  ...
  if(is_file($tempfile)){
    ...
    $update[_TABLE_FAQ_.'_PictureHome'] = "'".sql_safe($rootpathbanner)."'";
  ...
  if(!empty($putDataFile)){
    ...
    if(copy($putDataFile,$PathUploadPicture.$filename)) {
      ...
      $update[_TABLE_FAQ_.'_Picture'] = "'".sql_safe($filename)."'";
      $update[_TABLE_FAQ_.'_PictureAlt'] = "'".sql_safe($putData)."'";
      unlink($putDataFile);
    }
  }
  $update[_TABLE_FAQ_.'_DesDate'] = "'".sql_safe($inputDesDate)."'";
	$update[_TABLE_FAQ_.'_GID'] = "'".sql_safe($selectGroup)."'";
	$update[_TABLE_FAQ_."_LastUpdate"] = "NOW()";
	$update[_TABLE_FAQ_."_UpdateByID"] = sql_safe($_SESSION['Session_Admin_ID'],false,true);
	$update[_TABLE_FAQ_.'_Ignore'] = "'".sql_safe($Lang)."'";
	$update[_TABLE_FAQ_.'_Start'] = "'".sql_safe($inputStartDate)."'";
	$update[_TABLE_FAQ_.'_End'] = "'".sql_safe($inputExpireDate)."'";
	$update[_TABLE_FAQ_.'_IP'] = "'".get_real_ip()."'";
	$z = new __webctrl;
	$z->update(_TABLE_FAQ_,$update,array(_TABLE_FAQ_."_ID=" => (int)$itemid));
	unset($update);
}
echo 2;
CloseDB();
