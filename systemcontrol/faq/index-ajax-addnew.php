...
//
$sql = "SELECT MAX("._TABLE_FAQ_."_Order) AS MaxO FROM "._TABLE_FAQ_;
$sql .= " WHERE "._TABLE_FAQ_."_Key IN ('".implode("','",$dataModuleKey)."')";
$z = new __webctrl;
$z->sql($sql);
$v = $z->row();
$Row = $v[0];
...
$insert = array();
$insert[_TABLE_FAQ_.'_Key'] = "'".$mymenukey."'";
$insert[_TABLE_FAQ_.'_GID'] = "'".sql_safe($selectGroup)."'";
$insert[_TABLE_FAQ_.'_DesDate'] = "'".sql_safe($inputDesDate)."'";
$insert[_TABLE_FAQ_.'_CreateDate'] = "NOW()";
$insert[_TABLE_FAQ_.'_LastUpdate'] = "NOW()";
$insert[_TABLE_FAQ_.'_CreateByID'] = sql_safe($_SESSION['Session_Admin_ID'],false,true);
$insert[_TABLE_FAQ_.'_UpdateByID'] = sql_safe($_SESSION['Session_Admin_ID'],false,true);
$insert[_TABLE_FAQ_.'_Start'] = "'".sql_safe($inputStartDate)."'";
$insert[_TABLE_FAQ_.'_End'] = "'".sql_safe($inputExpireDate)."'";
$insert[_TABLE_FAQ_.'_Ignore'] = "'".sql_safe($Lang)."'";
$insert[_TABLE_FAQ_.'_IP'] = "'".get_real_ip()."'";
$insert[_TABLE_FAQ_.'_Order'] = sql_safe($MaxOrder,false,true);
$insert[_TABLE_FAQ_.'_Status'] = "'".sql_safe($ListStatus)."'";
$insert[_TABLE_FAQ_.'_StatusHome'] = "'".sql_safe($StatusHome)."'";
$insert[_TABLE_FAQ_.'_StatusRating'] = "'".sql_safe($ListRating)."'";
$insert[_TABLE_FAQ_.'_StatusComment'] = "'".sql_safe($ListComment)."'";
$insert[_TABLE_FAQ_.'_StatusRelate'] = "'".sql_safe($ListRelate)."'";
$insert[_TABLE_FAQ_.'_Pin'] = "'".sql_safe($StatusPin)."'";
$insert[_TABLE_FAQ_.'_StatusContentPassword'] = "'".sql_safe($ListStatusContentPassword)."'";
$insert[_TABLE_FAQ_.'_ContentPassword'] = "'".sql_safe($ContentPassword)."'";
$insert[_TABLE_FAQ_.'_Public'] = "'".sql_safe($StatusPublic)."'";
...
if(is_file($tempfile)){
  ...
  $insert[_TABLE_FAQ_.'_PictureHome'] = "'".sql_safe($rootpathbanner)."'";
}
...
if(!empty($putDataFile)){
  ...
  if(copy($putDataFile,$PathUploadPicture.$filename)) {
    ...
    $insert[_TABLE_FAQ_.'_Picture'] = "'".sql_safe($filename)."'";
    $insert[_TABLE_FAQ_.'_PictureAlt'] = "'".sql_safe($putData)."'";
    unlink($putDataFile);
  }
}

$z = new __webctrl;
$z->insert(_TABLE_FAQ_,$insert);
$insertid = $z->insertid();
unset($insert);
...
foreach($systemLang as $lkey=>$lval){
  ...
  $insert[_TABLE_FAQ_DETAIL_.'_ContentID'] = "'".sql_safe($insertid)."'";
  $insert[_TABLE_FAQ_DETAIL_.'_Lang'] = "'".sql_safe($lkey)."'";
  $insert[_TABLE_FAQ_DETAIL_.'_Subject'] = "'".sql_safe($inputSubject)."'";
  $insert[_TABLE_FAQ_DETAIL_.'_Title'] = "'".sql_safe($inputTitle)."'";
  $insert[_TABLE_FAQ_DETAIL_.'_HTMLFileName'] = "'".sql_safe($html01)."'";
  $insert[_TABLE_FAQ_DETAIL_.'_UpdateDate'] = "NOW()";
  if(!empty($Ignore)){
    $insert[_TABLE_FAQ_DETAIL_.'_Status'] = "'Off'";
    $Lang .= "|".$lkey.":Off";
  }else{
    $insert[_TABLE_FAQ_DETAIL_.'_Status'] = "'On'";
    $Lang .= "|".$lkey.":On";
  }
  $z = new __webctrl;
  $z->insert(_TABLE_FAQ_DETAIL_,$insert);
  unset($insert);
}
$sql = "UPDATE "._TABLE_FAQ_." SET "._TABLE_FAQ_."_Ignore='".sql_safe($Lang)."' WHERE "._TABLE_FAQ_."_ID = ".(int)$insertid;
$z = new __webctrl;
$z->query($sql);
