function loadpageajax(page){
	...
	html +='<tr>';
		...
		html +='<th class="w250">'+Array_Mod_Lang["txt:Country"][actionlang]+'</th>';
	...
	if(len>0){
		for (var i = 0; i< len; i++) {
			var timeopen = json['result'][i].vTime;
			...
			html +='<tr class="listitem" data-z="'+numberindex+'">';
				...
				html += '<td>';
					...
					if(groupheader>0){
						html +='<p class="">'+Array_Mod_Lang["txt:Group"][actionlang]+' : '+json['result'][i].valueGroupname+'</p>';
					}
					if(timeopen.length>0){
						$.each(timeopen, function( index, value ) {
							html += '<div class="timeopenclose">';
								html += '<div class="inday">'+value["DayName"]+'</div>';
								html += '<div class="indaydot">:</div>';
								html += '<div class="inopen">'+value["_Open"]+'</div>';
								html += '<div class="indaydot">-</div>';
								html += '<div class="inclose">'+value["_Close"]+'</div>';
							html += '</div>';
						});
					}
				html += '</td>';
				html += '<td class="text-left">'+json['result'][i].CountryName+'</td>';
...
function savelistorderContent(myval,pagesize){
  ...
  var FullPath = $('#ajaxFrm input[name=PathURL]').val();
    ...
    type:"POST",
    url:FullPath+'supermarket/index-dataaction.php',
...
function changeInPin(t){
	...
	$.ajax({
		...
		'type':'POST',
		'data': {MyData:mydata},
		'url': PathURL+"supermarket/index-dataaccess.php",
		'success': function (data) {
			td.html($.trim(data));
		},
		complete: function(){
