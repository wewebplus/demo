...
switch($myaction){
  case 'sortlist':
    ...
    $index = 0;
    $indexinline = 0;
    $sql = "SELECT COUNT(*) AS CountRecord FROM "._TABLE_SUPERMARKET_." WHERE 1";
    $z = new __webctrl;
    $z->sql($sql);
    $v = $z->row();
    ...
    //print_r($ar);
    foreach($ar as $k=>$v){
      $update[_TABLE_SUPERMARKET_."_Order"] = sql_safe($k,false,true);
      //$z = new __webctrl;
      $z->update(_TABLE_SUPERMARKET_,$update,array(_TABLE_SUPERMARKET_."_ID=" => intval($v)));
      unset($update);
    }
    echo 'OK';
