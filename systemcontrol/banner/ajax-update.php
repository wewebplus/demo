...
if($itemid>0){
  foreach($systemLang as $lkey=>$lval){
    ...
    if(is_file($tempfile)){
      ...
      $update[_TABLE_BANNER_DETAIL_.'_PictureFile'] = "'".sql_safe($rootpathbanner)."'";

      $insert[_TABLE_BANNER_DETAIL_.'_PictureFile'] = "'".sql_safe($rootpathbanner)."'";
      if($detailid>0){
        $sqlx = "SELECT "._TABLE_BANNER_DETAIL_."_PictureFile AS PictureFile FROM "._TABLE_BANNER_DETAIL_." WHERE "._TABLE_BANNER_DETAIL_."_ID = ".(int)$detailid;
    		$zx = new __webctrl;
    		$zx->sql($sqlx);
    		$vx = $zx->row();
    		$rowx = $vx[0];
    		$oldhtml01 = $PathUploadPicture.$rowx['PictureFile'];
        $oldhtml02 = $PathUploadPicture."thum-1-".$rowx['PictureFile'];
        $oldhtml03 = $PathUploadPicture."thum-1-".$rowx['PictureFile'].".webp";
  	...
  	if($detailid>0){
  		$update[_TABLE_BANNER_DETAIL_.'_Subject'] = "'".sql_safe($inputSubject)."'";
  		$update[_TABLE_BANNER_DETAIL_.'_Title'] = "'".sql_safe($inputTitle)."'";
      $update[_TABLE_BANNER_DETAIL_.'_BannerCode'] = "'".sql_safe($inputBannerCode)."'";
      $update[_TABLE_BANNER_DETAIL_.'_URL'] = "'".sql_safe($inputURL)."'";
      $update[_TABLE_BANNER_DETAIL_.'_Target'] = "'".sql_safe($selectTarget)."'";
  		$update[_TABLE_BANNER_DETAIL_.'_UpdateDate'] = "NOW()";
  		if(!empty($Ignore)){
  			$update[_TABLE_BANNER_DETAIL_.'_Status'] = "'Off'";
  			$Lang .= "|".$lkey.":Off";
  		}else{
  			$update[_TABLE_BANNER_DETAIL_.'_Status'] = "'On'";
  			$Lang .= "|".$lkey.":On";
  		}
  		$z = new __webctrl;
  		$z->update(_TABLE_BANNER_DETAIL_,$update,array(_TABLE_BANNER_DETAIL_."_ID=" => (int)$detailid));
  		unset($update);
  	}else{
  		$insert[_TABLE_BANNER_DETAIL_.'_ContentID'] = "'".sql_safe($itemid)."'";
  ...
  $update[_TABLE_BANNER_.'_GroupID'] = sql_safe($GroupID,false,true);
  $update[_TABLE_BANNER_.'_GroupBanner'] = "'".sql_safe($selectGroup)."'";
  $update[_TABLE_BANNER_.'_GroupW'] = "'".sql_safe($W)."'";
  $update[_TABLE_BANNER_.'_GroupH'] = "'".sql_safe($H)."'";
	$update[_TABLE_BANNER_."_LastUpdate"] = "NOW()";
	$update[_TABLE_BANNER_."_UpdateByID"] = sql_safe($_SESSION['Session_Admin_ID'],false,true);
	$update[_TABLE_BANNER_.'_Ignore'] = "'".sql_safe($Lang)."'";
	$update[_TABLE_BANNER_.'_Start'] = "'".sql_safe($inputStartDate)."'";
	$update[_TABLE_BANNER_.'_End'] = "'".sql_safe($inputExpireDate)."'";
	$z = new __webctrl;
	$z->update(_TABLE_BANNER_,$update,array(_TABLE_BANNER_."_ID=" => (int)$itemid));
	unset($update);
}
echo 2;
CloseDB();
