		...
		for (var i = 0; i< len; i++) {
			...
			html +='<tr class="listitem" data-z="'+numberindex+'">';
			  ...
			  html +='<td class="">'+json['result'][i].valuePicture+'</td>';
				html +='<td>';
				html +='<span>'+json['result'][i].valueGroupBanner+'</span>';
			  html +='</td>';
			  html +='<td>';
				html +='<span>'+json['result'][i].valueSubject+'</span>';
			  html +='</td>';
			  html +='<td class="">'+json['result'][i].valueDateshow+'</td>';
				html +='<td class="">'+json['result'][i].CountLogs+'</td>';
				html += '<td>';
...
function savelistorderContent(myval,pagesize){
  ...
  var FullPath = $('#ajaxFrm input[name=PathURL]').val();
    ...
    type:"POST",
    url:FullPath+'banner/index-dataaction.php',
...
function changeStatus(t){
	...
	$.ajax({
		...
		'type':'POST',
		'data': {MyData:mydata},
		'url': PathURL+"banner/index-dataaccess.php",
		'success': function (data) {
...
function ajaxuFileUploadProgress(t){
	var tb = $(t);
  ...
  var allowed_file_types 		= ['image/png', 'image/jpeg', 'image/pjpeg', 'image/gif'];
...
function changeTypeUser(t,txt=''){
	var tb = $(t);
	$('.dualselect').empty();
	$('.dualselect').html('<div id="dualSelectExample" style="width:100%; height:300px; padding:10px;"></div>');
	var mydata = tb.val();
	var json = (function () {
		var json = null;
		...
		$.ajax({
			...
			'global': false,
			'type':'POST',
			'data':{mydata:mydata,txtcheck:txt,LoginData:LoginData},
			'url': '../ajax/ajax-loadpagemember-json.php',
			'dataType': "json",
			'success': function (data) {
				json = data;
			},
	...
	var len = json['result'].length;
	if(len>0){
		dsl = $('#dualSelectExample').DualSelectList({
			'candidateItems' : json['result'],
			'selectionItems' : json['resultuser'],
			'colors' : {
				'itemText' : 'white',
...
function ClickPublic(t){
	var tb = $(t);
	if(tb.is(":checked")) {
		$('.mymemtype').removeClass('hidden');
		$('.mymemuser').removeClass('hidden');
	}else{
		$('.mymemtype').addClass('hidden');
		$('.mymemuser').addClass('hidden');
	}
}
function exportClickExcel(t){
	var PathURL = $('#ajaxFrm input[name=PathURL]').val();
	var tb = $(t);
	var searchIDs = tb.attr('rev');
	var url = PathURL+'banner/printexcelclick.php?'+searchIDs;
