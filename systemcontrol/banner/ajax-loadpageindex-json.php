...
$sql .= " (";
	...
	$arrf[] = "a."._TABLE_BANNER_."_Status AS ListStatus";
	$arrf[] = "a."._TABLE_BANNER_."_Order AS ListOrder";
	$arrf[] = "a."._TABLE_BANNER_."_Start AS StartDate";
	$arrf[] = "a."._TABLE_BANNER_."_End AS EndDate";
  $arrf[] = "a."._TABLE_BANNER_."_GroupID AS GroupID";
  ...
  $sql .= " LEFT JOIN (";
    ...
    $sql .= "(";
      ...
      $arrinnercount[] = _TABLE_BANNER_LOGS_."_BannerID AS JoinContentID";
      $sql .= "SELECT ".implode(',',$arrinnercount)." FROM "._TABLE_BANNER_LOGS_." WHERE 1 GROUP BY "._TABLE_BANNER_LOGS_."_BannerID";
      unset($arrinnercount);
    $sql .= ") TBCount";
  $sql .= ") TBJoinView ON (a."._TABLE_BANNER_."_ID = TBJoinView.JoinContentID)";
	// $sql .= " WHERE a."._TABLE_BANNER_."_Key='".$Login_MenuID."'";
  $sql .= " WHERE a."._TABLE_BANNER_."_Key IN ('".implode("','",$dataModuleKey)."')";
	unset($arrf);
$sql .= ") TBmain";
$sql .= " WHERE 1";
if(!empty($dataGroup)){
...
if($RecordCount>0) {
    ...
    $CountLogs = $Row["CountRefView"];
      ...
      $listmnubtn .='<li>';
        $dataexportexcel = encode_URL('Login_MenuID='.$Login_MenuID.'&itemid='.$ID.'&actiontype=listexportexcel');
        $listmnubtn .='<a rev="'.$dataexportexcel.'" rel="'.$index.'" href="javascript:void(0);" onclick="exportClickExcel(this);">Export Click</a>';
      $listmnubtn .='</li>';
			$listmnubtn .='<li>';
