...
function loadpageajax(page){
	...
	var json = (function () {
		...
		$.ajax({
			...
			'url': 'index-ajax-loadpage-json.php',
			'dataType': "json",
  ...
  html +='<thead>';
    html +='<tr>';
			html +='<th class="w40 text-center">';
				...
				html +='</label>';
			html += '</th>';
      html +='<th class="">'+Array_Mod_Lang["txt:Management Name"][actionlang]+'</th>';
			html +='<th class="w175 text-left">'+Array_Mod_Lang["txt:Management Code"][actionlang]+'</th>';
			html +='<th class="w175 text-left">'+Array_Mod_Lang["txt:Management Shot Code"][actionlang]+'</th>';
      html +='<th class="w175 text-left">'+Array_Mod_Lang["txt:Management ISD Code"][actionlang]+'</th>';
			html +='<th class="w175 text-center">'+Array_Mod_Lang["txt:Management Status"][actionlang]+'</th>';
			html +='<th class="w175 text-center">'+Array_Mod_Lang["txt:Management Action"][actionlang]+'</th>';
    html +='</tr>';
  html +='</thead>';
  html +='<tbody>';
	if(len>0){
		for (var i = 0; i< len; i++) {
			...
			var countRef = parseInt(json['result'][i].CountRefCheck);
			var ShowDropdown = json['result'][i].LinkDhowDropdown;
			html +='<tr>';
				...
				html +='</td>';
			  html +='<td>';
				html +='<span>'+json['result'][i].CountryNameEN+'</span>';
			  html +='</td>';
			  html +='<td class="">'+json['result'][i].CountryLongCode+'</td>';
				html +='<td class="">'+json['result'][i].CountryCode+'</td>';
				html +='<td class="">'+json['result'][i].CountryISD+'</td>';
			  ...
			  html +='</td>';
				html +='<td class="text-center">';
					html += '<div class="btn-group">';
						// html += '<a href="javascript:void(0)" rev="'+json['result'][i].LinkUser+'" class="btn btn-primary dark" onclick="clicktoaction(this);"><i class="fa fa-user-plus"></i></a>';
	            ...
	            action: function () {
								var json = (function () {
									...
									$.ajax({
										...
										'url': FullPath+'world_wide_management/index-dataaction.php',
										'dataType': "json",
										'success': function (data) {
											json = data;
										},
...
function changeStatus(t){
	...
	$.ajax({
		...
		'type':'POST',
		'data': {MyData:mydata},
		'url': PathURL+"world_wide_management/index-dataaccess.php",
		'success': function (data) {
...
function checktpyingeng(e){
	var englishAlphabetAndWhiteSpace = /[a-zA-Z0-9-_]/g;
	var str = String.fromCharCode(e.which);
	if (e.keyCode == 8 || e.keyCode == 37 || e.keyCode == 39 || englishAlphabetAndWhiteSpace.test(str)) {
...
function chkdatashotname(t){
	...
	var json = (function () {
		...
		$.ajax({
			...
			'global': false,
			'type':'POST',
			'data':{myaction:'checkshotname',LoginData:LoginData,indata:indata},
			'url': PathURL+"world_wide_management/index-dataaction.php",
			'dataType': "json",
			'success': function (data) {
				json = data;
			},
	...
	if(result){
		$('#ErrorShotname').html(json['Intext']);
		$('#ErrorShotname').addClass('text-primary');
		$('#ErrorShotname').removeClass('text-danger');
		$('button[type=submit]').prop( "disabled", false);
	}else{
		$('#ErrorShotname').html(json['Intext']);
		$('button[type=submit]').prop( "disabled", true);
		$('#ErrorShotname').addClass('text-danger');
		$('#ErrorShotname').removeClass('text-primary');
		tb.focusTextToEnd();
	}
}
