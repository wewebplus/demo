	            ...
	            action: function () {
								var json = (function () {
									...
									$.ajax({
										...
										'url': FullPath+'recipes/group-dataaction.php',
										'dataType': "json",
										'success': function (data) {
											json = data;
										},
...
function changeStatus(t){
	...
	$.ajax({
		...
		'type':'POST',
		'data': {MyData:mydata},
		'url': PathURL+"recipes/group-dataaccess.php",
		'success': function (data) {
...
function clicktoexportinfo(t){
	...
	var url = PathURL+'recipes/printexcelcontact.php?'+LoginData;
...
function exportGroupExcel(t){
	...
	var url = PathURL+'recipes/printexcelcontactgroup.php?'+searchIDs;
	var triggerDownload = $("<a>").attr("href", url).attr('target', '_blank').appendTo("body");
	triggerDownload[0].click();
	triggerDownload.remove();
}
