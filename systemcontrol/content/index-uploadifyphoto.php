...
foreach($_FILES as $index => $file){
  ...
  if(!empty($fileTempName) && is_uploaded_file($fileTempName))  {
    ...
    $sql = "SELECT MAX("._TABLE_CONTENT_PIC_."_Order) AS MaxO FROM "._TABLE_CONTENT_PIC_." WHERE 1 ";
		$sql .= " AND "._TABLE_CONTENT_PIC_."_ContentID = ".(int)$itemid;
		if($sessionid<>""){
			$sql .= " AND "._TABLE_CONTENT_PIC_."_Session = '".$sessionid."'";
		}
		$z = new __webctrl;
		$z->sql($sql);
		$v = $z->row();
		$Row = $v[0];
		$MaxOrder = $Row["MaxO"]+1;
		$insert[_TABLE_CONTENT_PIC_."_ContentID"] = "'".sql_safe($itemid)."'";
		$insert[_TABLE_CONTENT_PIC_."_Session"] = "'".sql_safe($sessionid)."'";
		$insert[_TABLE_CONTENT_PIC_."_Subject"] = "'".sql_safe($fileName)."'";
		$insert[_TABLE_CONTENT_PIC_."_FileName"] = "'".sql_safe($NewFile)."'";
		$insert[_TABLE_CONTENT_PIC_."_Views"] = "'0'";
		$insert[_TABLE_CONTENT_PIC_."_CreateByID"] = "'".(int)$_SESSION['Session_Admin_ID']."'";
		$insert[_TABLE_CONTENT_PIC_."_CreateDate"] = "NOW()";
		$insert[_TABLE_CONTENT_PIC_."_Order"] = "'".sql_safe($MaxOrder)."'";
		$z = new __webctrl;
		$z->insert(_TABLE_CONTENT_PIC_,$insert);
		unset($insert);
