      ...
      //var numberindex = json['result'][i].ListIndex;
			...
			html +='<tr class="listitem" data-z="'+numberindex+'">';
				...
				html += '<td>';
					...
					html += '<p>'+json['result'][i].valueTitle+'</p>';
					if(groupheader>0){
						html += '<p>'+Array_Mod_Lang["txt:Group"][actionlang]+' : '+json['result'][i].valueGroupname+'</p>';
					}
					html += '<p>'+Array_Mod_Lang["txt:Date"][actionlang]+' : '+json['result'][i].valueDateshow+'</p>';
				html += '</td>';
				...
				html += '<td class="text-left liststatus">';
					html += '<div class="divstatus">';
						...
						html += '<div>'+json['result'][i].btnRating+'</div>';
						html += '<div>'+json['result'][i].btnComment+'</div>';
						html += '<div>'+json['result'][i].btnstatushome+'</div>';
...
function savelistorderContent(myval,pagesize){
  ...
  var FullPath = $('#ajaxFrm input[name=PathURL]').val();
    ...
    type:"POST",
    url:FullPath+'content/index-dataaction.php',
