...
//
$sql = "SELECT MAX("._TABLE_CONTENT_."_Order) AS MaxO FROM "._TABLE_CONTENT_;
$sql .= " WHERE "._TABLE_CONTENT_."_Key IN ('".implode("','",$dataModuleKey)."')";
$z = new __webctrl;
$z->sql($sql);
$v = $z->row();
$Row = $v[0];
...
$insert = array();
$insert[_TABLE_CONTENT_.'_Key'] = "'".$mymenukey."'";
$insert[_TABLE_CONTENT_.'_GID'] = "'".sql_safe($selectGroup)."'";
$insert[_TABLE_CONTENT_.'_DesDate'] = "'".sql_safe($inputDesDate)."'";
$insert[_TABLE_CONTENT_.'_CreateDate'] = "NOW()";
$insert[_TABLE_CONTENT_.'_LastUpdate'] = "NOW()";
$insert[_TABLE_CONTENT_.'_CreateByID'] = sql_safe($_SESSION['Session_Admin_ID'],false,true);
$insert[_TABLE_CONTENT_.'_UpdateByID'] = sql_safe($_SESSION['Session_Admin_ID'],false,true);
$insert[_TABLE_CONTENT_.'_Start'] = "'".sql_safe($inputStartDate)."'";
$insert[_TABLE_CONTENT_.'_End'] = "'".sql_safe($inputExpireDate)."'";
$insert[_TABLE_CONTENT_.'_Ignore'] = "'".sql_safe($Lang)."'";
$insert[_TABLE_CONTENT_.'_IP'] = "'".get_real_ip()."'";
$insert[_TABLE_CONTENT_.'_Order'] = sql_safe($MaxOrder,false,true);
$insert[_TABLE_CONTENT_.'_Status'] = "'".sql_safe($ListStatus)."'";
$insert[_TABLE_CONTENT_.'_StatusHome'] = "'".sql_safe($StatusHome)."'";
$insert[_TABLE_CONTENT_.'_StatusRating'] = "'".sql_safe($ListRating)."'";
$insert[_TABLE_CONTENT_.'_StatusComment'] = "'".sql_safe($ListComment)."'";
$insert[_TABLE_CONTENT_.'_StatusRelate'] = "'".sql_safe($ListRelate)."'";
$insert[_TABLE_CONTENT_.'_Pin'] = "'".sql_safe($StatusPin)."'";
$insert[_TABLE_CONTENT_.'_StatusContentPassword'] = "'".sql_safe($ListStatusContentPassword)."'";
$insert[_TABLE_CONTENT_.'_ContentPassword'] = "'".sql_safe($ContentPassword)."'";
$insert[_TABLE_CONTENT_.'_Public'] = "'".sql_safe($StatusPublic)."'";
if(isset($dataOption["MenuID"])){
  if(intval($dataOption["MenuID"])>0){
    $SiteID = 1;
    $insert[_TABLE_CONTENT_.'_SiteID'] = sql_safe($SiteID,false,true);
    $insert[_TABLE_CONTENT_.'_MenuID'] = sql_safe($dataOption["MenuID"],false,true);
  }
}
...
if(is_file($tempfile)){
  ...
  $insert[_TABLE_CONTENT_.'_PictureHome'] = "'".sql_safe($rootpathbanner)."'";
}
...
if(!empty($putDataFile)){
  ...
  if(copy($putDataFile,$PathUploadPicture.$filename)) {
    ...
    $insert[_TABLE_CONTENT_.'_Picture'] = "'".sql_safe($filename)."'";
    $insert[_TABLE_CONTENT_.'_PictureAlt'] = "'".sql_safe($putData)."'";
    unlink($putDataFile);
  }
}

$z = new __webctrl;
$z->insert(_TABLE_CONTENT_,$insert);
$insertid = $z->insertid();
unset($insert);
...
foreach($systemLang as $lkey=>$lval){
  ...
  $insert[_TABLE_CONTENT_DETAIL_.'_ContentID'] = "'".sql_safe($insertid)."'";
...
$sql = "UPDATE "._TABLE_CONTENT_." SET "._TABLE_CONTENT_."_Ignore='".sql_safe($Lang)."' WHERE "._TABLE_CONTENT_."_ID = ".(int)$insertid;
$z = new __webctrl;
$z->query($sql);
