...
if($itemid>0){
  foreach($systemLang as $lkey=>$lval){
    ...
    if($detailid>0){
      $update[_TABLE_CONTACT_GROUP_DETAIL_.'_Subject'] = "'".sql_safe($inputGroupSubject)."'";
      $update[_TABLE_CONTACT_GROUP_DETAIL_.'_Email'] = "'".sql_safe($inputGroupEmail)."'";
      $update[_TABLE_CONTACT_GROUP_DETAIL_.'_Title'] = "'".sql_safe($inputGroupTitle)."'";
      $update[_TABLE_CONTACT_GROUP_DETAIL_.'_UpdateDate'] = "NOW()";
      if(!empty($Ignore)){
        $update[_TABLE_CONTACT_GROUP_DETAIL_.'_Status'] = "'Off'";
        $Lang .= "|".$lkey.":Off";
      }else{
        $update[_TABLE_CONTACT_GROUP_DETAIL_.'_Status'] = "'On'";
        $Lang .= "|".$lkey.":On";
      }
      $z = new __webctrl;
      $z->update(_TABLE_CONTACT_GROUP_DETAIL_,$update,array(_TABLE_CONTACT_GROUP_DETAIL_."_ID=" => (int)$detailid));
      unset($update);
    }else{
      $insert[_TABLE_CONTACT_GROUP_DETAIL_.'_ContentID'] = "'".sql_safe($itemid)."'";
	...
	$update[_TABLE_CONTACT_GROUP_."_LastUpdate"] = "NOW()";
	$update[_TABLE_CONTACT_GROUP_."_UpdateByID"] = sql_safe($_SESSION['Session_Admin_ID'],false,true);
	$update[_TABLE_CONTACT_GROUP_.'_Ignore'] = "'".sql_safe($Lang)."'";
	$update[_TABLE_CONTACT_GROUP_.'_IP'] = "'".get_real_ip()."'";
	$z = new __webctrl;
	$z->update(_TABLE_CONTACT_GROUP_,$update,array(_TABLE_CONTACT_GROUP_."_ID=" => (int)$itemid));
	unset($update);
}
echo 2;
CloseDB();
