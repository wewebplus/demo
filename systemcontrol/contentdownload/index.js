function loadpageajax(page){
  ...
  html +='<thead>';
    html +='<tr>';
		...
		html +='<th class="w50">'+Array_Mod_Lang["txt:File"][actionlang]+'</th>';
		html +='<th class="w50">'+Array_Mod_Lang["txt:Dwn"][actionlang]+'</th>';
		html +='<th  class="w125 text-left">'+Array_Lang["txt:Status"][actionlang]+'</th>';
		...
		for (var i = 0; i< len; i++) {
			...
			html +='<tr class="listitem" data-z="'+numberindex+'">';
			  ...
			  html +='<td>';
					...
					if(groupheader>0){
						html += '<p>'+Array_Mod_Lang["txt:Group"][actionlang]+' : '+json['result'][i].valueGroupname+'</p>';
					}
			  html +='</td>';
			  html +='<td class="">'+json['result'][i].valueDateshow+'</td>';
				html += '<td class="text-center">'+json['result'][i].CountFile+'</td>';
				html += '<td class="text-center">'+json['result'][i].CountDWN+'</td>';
...
function savelistorderContent(myval,pagesize){
  ...
  var FullPath = $('#ajaxFrm input[name=PathURL]').val();
    ...
    type:"POST",
    url:FullPath+'contentdownload/index-dataaction.php',
...
function changeStatus(t){
	...
	$.ajax({
		...
		'type':'POST',
		'data': {MyData:mydata},
		'url': PathURL+"contentdownload/index-dataaccess.php",
		'success': function (data) {
