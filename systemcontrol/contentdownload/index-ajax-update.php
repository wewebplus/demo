...
if($itemid>0){
  foreach($systemLang as $lkey=>$lval){
    ...
    if(is_file($tmpfile)){
      $sqlx = "SELECT "._TABLE_DOWNLOAD_DETAIL_."_F AS DwnFile FROM "._TABLE_DOWNLOAD_DETAIL_." WHERE "._TABLE_DOWNLOAD_DETAIL_."_ID = ".(int)$detailid;
  		$zx = new __webctrl;
  		$zx->sql($sqlx);
  		$vx = $zx->row();
  		$rowx = $vx[0];
  		$oldvdo = $PathUploadFile.$rowx['DwnFile'];
      ...
      $update[_TABLE_DOWNLOAD_DETAIL_.'_F'] = "'".sql_safe($ushowfile)."'";
      $update[_TABLE_DOWNLOAD_DETAIL_.'_FType'] = "'".sql_safe($myExtension)."'";
      $update[_TABLE_DOWNLOAD_DETAIL_.'_FName'] = "'".sql_safe($ushowfilename)."'";
  	...
  	if($detailid>0){
  		...
  		$update[_TABLE_DOWNLOAD_DETAIL_.'_Subject'] = "'".sql_safe($inputSubject)."'";
  		$update[_TABLE_DOWNLOAD_DETAIL_.'_Title'] = "'".sql_safe($inputTitle)."'";
      $update[_TABLE_DOWNLOAD_DETAIL_.'_Type'] = "'".sql_safe($VDOType)."'";
      $update[_TABLE_DOWNLOAD_DETAIL_.'_L'] = "'".sql_safe($inputLink)."'";
  		$update[_TABLE_DOWNLOAD_DETAIL_.'_UpdateDate'] = "NOW()";
  		if(!empty($Ignore)){
  			$update[_TABLE_DOWNLOAD_DETAIL_.'_Status'] = "'Off'";
  			$Lang .= "|".$lkey.":Off";
  		}else{
  			$update[_TABLE_DOWNLOAD_DETAIL_.'_Status'] = "'On'";
  			$Lang .= "|".$lkey.":On";
  		}
  		$z = new __webctrl;
  		$z->update(_TABLE_DOWNLOAD_DETAIL_,$update,array(_TABLE_DOWNLOAD_DETAIL_."_ID=" => (int)$detailid));
  		unset($update);
  	}else{
  		$insert[_TABLE_DOWNLOAD_DETAIL_.'_ContentID'] = "'".sql_safe($itemid)."'";
  ...
  $sql = "SELECT "._TABLE_DOWNLOAD_."_Picture AS thumb FROM "._TABLE_DOWNLOAD_." WHERE "._TABLE_DOWNLOAD_."_ID = ".(int)$itemid;
  $z = new __webctrl;
  $z->sql($sql);
  $v = $z->row();
  $row = $v[0];
  ...
  if(!empty($putDataFile)){
    ...
    if(copy($putDataFile,$PathUploadPicture.$filename)) {
      ...
      $update[_TABLE_DOWNLOAD_.'_Picture'] = "'".sql_safe($filename)."'";
      $update[_TABLE_DOWNLOAD_.'_PictureAlt'] = "'".sql_safe($putData)."'";
      unlink($putDataFile);
    }
  }

	$update[_TABLE_DOWNLOAD_.'_GID'] = "'".sql_safe($selectGroup)."'";
  $update[_TABLE_DOWNLOAD_.'_SGID'] = "'".sql_safe($selectSubGroup)."'";
	$update[_TABLE_DOWNLOAD_."_LastUpdate"] = "NOW()";
	$update[_TABLE_DOWNLOAD_."_UpdateByID"] = sql_safe($_SESSION['Session_Admin_ID'],false,true);
	$update[_TABLE_DOWNLOAD_.'_Ignore'] = "'".sql_safe($Lang)."'";
	$update[_TABLE_DOWNLOAD_.'_Start'] = "'".sql_safe($inputStartDate)."'";
	$update[_TABLE_DOWNLOAD_.'_End'] = "'".sql_safe($inputExpireDate)."'";
	$update[_TABLE_DOWNLOAD_.'_IP'] = "'".get_real_ip()."'";
  $update[_TABLE_DOWNLOAD_.'_StatusContentPassword'] = "'".sql_safe($ListStatusContentPassword)."'";
  $update[_TABLE_DOWNLOAD_.'_ContentPassword'] = "'".sql_safe($ContentPassword)."'";
  $update[_TABLE_DOWNLOAD_.'_Public'] = "'".sql_safe($StatusPublic)."'";
	$z = new __webctrl;
	$z->update(_TABLE_DOWNLOAD_,$update,array(_TABLE_DOWNLOAD_."_ID=" => (int)$itemid));
	unset($update);
