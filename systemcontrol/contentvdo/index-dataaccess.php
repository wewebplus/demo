...
switch($actiontype){
	case 'changethisstatus':
		...
		$sql = "SELECT "._TABLE_VDO_."_Key AS myKey,"._TABLE_VDO_."_ID AS ID,"._TABLE_VDO_."_Status AS ListStatus FROM "._TABLE_VDO_." WHERE "._TABLE_VDO_."_ID = ".(int)$itemid;
		$z = new __webctrl;
		$z->sql($sql);
		$v = $z->row();
		$Row = $v[0];
		...
		$update[_TABLE_VDO_."_Status"] = "'".$StatusTo."'";
		$z = new __webctrl;
		$z->update(_TABLE_VDO_,$update,array(_TABLE_VDO_."_ID=" => $itemid));
		unset($update);
    ...
    $mymenukey = @$menuModuleKey[$indexLogin_MenuID];
		...
		$sql = "SELECT "._TABLE_VDO_DETAIL_."_VdoF AS VDOFile FROM "._TABLE_VDO_DETAIL_." WHERE "._TABLE_VDO_DETAIL_."_ContentID = ".(int)$itemid;
		$z = new __webctrl;
		$z->sql($sql);
		$num = $z->num();
		$r = $z->row();
	...
	break;case 'sort':
		...
		if($counArr>0){
			for($i=1;$i<$counArr;$i++){
				...
				if($value <> 'null'){
					$j = $counArr-$i;
					$update[_TABLE_VDO_."_Order"] = sql_safe($j,false,true);
					$z = new __webctrl;
					$z->update(_TABLE_VDO_,$update,array(_TABLE_VDO_."_ID=" => (int)$value));
					unset($update);
				}
			}
		}
