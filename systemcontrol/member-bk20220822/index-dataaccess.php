...
switch($actiontype){
	case 'changethisstatus':
		...
		$sql = "SELECT "._TABLE_MEMBER_."_ID AS ID,"._TABLE_MEMBER_."_Status AS ListStatus FROM "._TABLE_MEMBER_." WHERE "._TABLE_MEMBER_."_ID = ".(int)$itemid;
		$z = new __webctrl;
		$z->sql($sql);
		$v = $z->row();
		$Row = $v[0];
		...
		$update[_TABLE_MEMBER_."_Status"] = "'".$StatusTo."'";
		$z = new __webctrl;
		$z->update(_TABLE_MEMBER_,$update,array(_TABLE_MEMBER_."_ID=" => $itemid));
		unset($update);
	...
	break;case 'delete':
		...
		$sql = "SELECT "._TABLE_MEMBER_."_PictureFile AS thumb FROM "._TABLE_MEMBER_." WHERE "._TABLE_MEMBER_."_ID = ".(int)$itemid;
	  $z = new __webctrl;
	  $z->sql($sql);
	  $v = $z->row();
	  $row = $v[0];
    ...
    if(is_file($unlinkfile)){unlink($unlinkfile);}
		$z = new __webctrl;
		$z->del(_TABLE_MEMBER_,array(_TABLE_MEMBER_."_ID=" => (int)$itemid));
		echo "OK";
	break; default:
}
CloseDB();
