...
$sql .= " (";
	...
	$arrf[] = "a."._TABLE_INTRO_.'_Status AS ListStatus';
	$arrf[] = "a."._TABLE_INTRO_.'_Order AS ListOrder';
	$arrf[] = "a."._TABLE_INTRO_.'_Start AS StartDate';
	$arrf[] = "a."._TABLE_INTRO_.'_End AS EndDate';
  ...
  $sql .= " WHERE a."._TABLE_INTRO_."_Key IN ('".implode("','",$dataModuleKey)."')";
	unset($arrf);
$sql .= ") TBmain";
$sql .= " WHERE 1";
if(!empty($dataGroup)){
    ...
    if($LangStatus=='On'){
      ...
      $Fullname = (!empty($Row["SubjectEN"])?$Row["SubjectEN"]:'');
		...
		if(strtolower($myExtension)=='gif'){
			...
			$Picture = $PathUploadPicture.$PictureFile;//_RELATIVE_ACTIVITY_PICTURE_UPLOAD_.$Row["Picture"];
			if(is_file($Picture)){
				$showPicture = str_replace(_RELATIVE_PATH_UPLOAD_,_HTTP_PATH_UPLOAD_,$Picture);
				$showPicture = '<img src="'.$showPicture.'" class="imglist" alt="'.$Fullname.'" />';
			}else{
