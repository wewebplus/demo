...
class __webctrl
{
    ...
    public function __construct()
    {
       ...
       // mysqli_report( MYSQLI_REPORT_STRICT );
        try {
			...
			$optiondb = array(PDO::MYSQL_ATTR_FOUND_ROWS => true,PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION,PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8");
			$this->link = new PDO('mysql:host='.$host.';dbname='._DB_NAME_, _DB_USERNAME_, _DB_PASSWORD_,$optiondb);
	...
	public function block_injection($str){
		$str = htmlspecialchars($str,ENT_QUOTES);
		$str = $this->link->quote($str);
		return stripslashes($str);
	}
	
	public function chkQuote($str){
    ...
    public function query( $query ){
        ...
        if( $this->link->errorCode()!='00000'){
            $this->log_db_errors( $this->link->errorInfo(), $query );
            return false; 
        }else{
            return true;
        }
    ...
    public function get_row( $query, $object = false )
    {
        ...
        {
            $r = ( !$object ) ? $row->fetchAll() : $row->fetchObject();
            return $r;   
        }
    }
    ...
    public function get_results( $query, $object = false )
    {
        ...
        {
            $row = array();
            while( $r = ( !$object ) ? $results->fetch() : $results->fetchObject() )
            {
                $row[] = $r;
            }
            return $row;   
    ...
    public function insert( $table, $variables = array() )
    {
        ...
        {
            //return false; 
            $this->log_db_errors( $this->link->errorInfo(), $sql );
            return false;
        }
        else
        {
    ...
    */
    ...
    public function update( $table, $variables = array(), $where = array(), $limit = '' )
    {
        ...
        foreach( $variables as $field => $value )
        {
			...
			$updates[] = "$field = ".$this->chkQuote($value);
        }
        $sql .= implode(', ', $updates);
     ...
     *
     */   
    public function del( $table, $where = array(), $limit = '' )
    {
        self::$counter++;
        //Delete clauses require a where param, otherwise use "truncate"
        if( empty( $where ) )
        ...
        foreach( $where as $field => $value )
        {
            ...
            //$clause[] = "$field = '$value'";
			//$clause[] = "$field '$value'";
			$clause[] = str_replace("=","",$field)." = '$value'";
        }
        $sql .= " WHERE ". implode(' AND ', $clause);
    ...
    public function list_fields( $query )
    {
		...
		for($ii=0;$ii<$fields;$ii++){
			$listed_fields[$ii] = $query->getColumnMeta($ii);
		}
    ...
    public function num()
    {
        ...
        if( $this->link->errorCode()!="00000"){
			...
			$this->total_num = $num_rows->rowCount();
            return $num_rows->rowCount();
        }
    }
    public function numinpage()
    {
        self::$counter++;
		$query = $this->sql;
        $num_rows = $this->link->query( $query );
