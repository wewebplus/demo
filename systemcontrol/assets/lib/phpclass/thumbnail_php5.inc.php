...
class Thumbnail {
	...
	public function watermark($filename, $width=0, $height=0, $valign = 'bottom', $halign = 'left', $vmargin = 0, $hmargin = 0)
    {
            ...
            $this->workingImage != false)
        {
			...
			if($width>0 or $height>0){
				$watermarkObj = new Thumbnail($filename);
				$watermarkObj->resize($width,$height);
				$im = $watermarkObj->getObjectImage();
				$image = array($im,$watermarkObj->getCurrentWidth(),$watermarkObj->getCurrentHeight());
			}else{
            ...
            if ($image != false)
            {
                ...
                $posV = $this->calculatePosition($this->getCurrentWidth(),
                                                 $this->watermarkWidth,
                                                 $valign);
                // for height pos
                $posH = $this->calculatePosition($this->getCurrentHeight(),
                                                 $this->watermarkHeight,
                                                 $halign);
                                                 
                // insert watermark into imageobject
                imagecopyresampled($this->workingImage, 
